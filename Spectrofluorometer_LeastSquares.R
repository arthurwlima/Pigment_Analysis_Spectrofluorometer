...
for (i in 1:length(d$Amostra)){
	...
	if (file.exists(as.character(d[i,9]))){
	  ...
	  # Separando os valores de volume filtrado d[i,4]- "Volume_Coleta_L"  e 
		...
		# Selecionado apenas os valores de emissao. 
		# Valores de excitacao devem ser os mesmos entre referencias, Branco e amostra
		listJ <- seq(2,dim(dd)[2]-1, by=2)
	
		# ListaEmissao de acordo com as emissoes lidas nos padroes de pigmentos: 51 valores de excitacao
		# Amostras foram lidas com 52 excitacoes
		for(j in 1:51){
		  for (k in listJ){
...
print("Amostras encerradas. Merci Neveux.")
